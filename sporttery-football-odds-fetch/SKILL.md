...
description: 从中国体育彩票移动端足球计算器页面获取竞彩足球赔率，并更新项目内体彩赔率表 Markdown 文件。用于需要抓取或刷新体彩足球胜平负 HAD、让球胜平负 HHAD、比分 CRS、进球数 TTG 赔率，处理 m.sporttery.cn/mjc/jsq/zqspf/、webapi.sporttery.cn 接口、赔率缺失填充为 -、让球数写回表格等任务。
---
...
若某个进球数字段不存在、为空字符串，或 `ttg` 为空对象，写 `-`。
